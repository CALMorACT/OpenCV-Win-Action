...
async function run() {
  try {
    ...
    if (core.getInput('cached') !== 'true') {
      ...
      await exec.exec(
        `${pythonName} -m pip install "py7zr${core.getInput('py7zrversion')}"`
      )
      await exec.exec(
        `${pythonName} -m pip install "aqtinstall${core.getInput(
          'aqtversion'
        )}"`
      )
      ...
      if (tools) {
        tools.split(' ').forEach(async element => {
          ...
          await exec.exec(
            `${pythonName} -m aqt tool ${host} ${elements[0]} ${elements[1]} ${elements[2]}`,
            extraArgs
          )
        })
      }
    ...
    if (setEnv == 'true') {
      ...
      if (process.platform == 'linux') {
        if (process.env.LD_LIBRARY_PATH) {
          core.exportVariable(
            'LD_LIBRARY_PATH',
            process.env.LD_LIBRARY_PATH + ':' + qtPath + '/lib'
          )
        } else {
